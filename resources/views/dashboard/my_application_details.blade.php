        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="item-card flex flex-row gap-y-10 justify-between md:items-center">
                    ...
                    <div class="hidden md:flex flex-col">
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">
                            Rp {{ number_format($jobCandidate->job->salary, 0, ',', '.') }}/month
                        </h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                    ...
                    <div class="flex flex-row items-center gap-x-3">
                        ...
                        <div class="flex flex-col">
                            ...
                            <p class="text-slate-500 text-sm">{{ $jobCandidate->profile->occupation }} -
                                {{ $jobCandidate->profile->experience }} years experience
                            </p>
                        </div>
                    </div>
                ...
                @if ($jobCandidate->candidate_id == Auth::id() && $jobCandidate->is_hired && !$jobCandidate->job->is_open)
                    <hr class="my-5">
                    <h3 class="text-indigo-950 text-xl font-bold">Congrats! You've been selected!</h3>
                    <p>
                        Keep an eye on your email for further instructions from {{ $jobCandidate->job->company->name }}
                ...
                @elseif ($jobCandidate->candidate_id == Auth::id() && !$jobCandidate->is_hired && !$jobCandidate->job->is_open)
                    <hr class="my-5">
                    <h3 class="text-indigo-950 text-xl font-bold">Unfortunately, you have not been selected for this
                        position.</h3>
                    <p>Please feel free to check and apply for other openings that match your skills.</p>
                    <p>
                        <a href="{{ route('front.index') }}"
                            class="font-bold py-3 px-10 rounded-full bg-indigo-700 text-white">
                            Explore Jobs</a>
                    </p>
                @endif
            </div>
